...
static noinline long create_kqueue(request_t request){
    ...
    if(request.data_size>MAX_DATA_SIZE)     // MAX_DATA_SIZE = 0x20
...
static noinline long edit_kqueue(request_t request){
    ...
    for(i=1;i<queue->max_entries+1;i++){
        ...
        if(kqueue_entry && request.data && queue->data_size){           // 修改下标为 request.entry_idx 的 kqueue_entry->data
    ...
    if(request.entry_idx==0 && kqueue_entry && request.data && queue->data_size){   // 下标为0 则修改 queue->data
...
static noinline long save_kqueue_entries(request_t request){
    ...
    if(queue->data && request.data_size)        // request.data_size 大小未经验证，可以很大，然后泄露出来
